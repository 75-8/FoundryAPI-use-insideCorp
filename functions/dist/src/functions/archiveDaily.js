...
exports.archiveDailyHandler = archiveDailyHandler;
 ...
 */
async function archiveDailyHandler(myTimer, context) {
    ...
    try {
        ...
        for await (const blob of analyticsContainer.listBlobsFlat({ prefix })) {
            ...
            try {
                ...
                context.warn(`archiveDaily: Failed to read/parse analytics blob ${blob.name}: ${err instanceof Error ? err.message : String(err)}`);
        ...
        const parquetBuffer = await (0, parquetWriter_js_1.convertToParquetBuffer)(records);
        ...
        await (0, parquetWriter_js_1.uploadParquetToArchive)(archiveBlobClient, parquetBuffer);
        context.log(`archiveDaily: Parquet uploaded to archive/${archiveBlobPath} (Archive Tier)`);
    }
    catch (err) {
        // 処理失敗 → Poison Blob に記録 (§11)
        context.error(`archiveDaily: Failed to process archive for ${dateStr}: ${err instanceof Error ? err.message : String(err)}`);
        try {
            await (0, poisonBlobWriter_js_1.writePoisonBlob)(poisonContainer, 'archiveDaily', `Failed to generate/upload archive Parquet for ${dateStr}: ${err instanceof Error ? err.message : String(err)}`, { date: dateStr }, {
            ...
            context.error(`archiveDaily: CRITICAL - Failed to write Poison Blob for ${dateStr}: ${poisonErr instanceof Error ? poisonErr.message : String(poisonErr)}`);
        }
 ...
 */
functions_1.app.timer('archiveDaily', {
...
//# sourceMappingURL=archiveDaily.js.map