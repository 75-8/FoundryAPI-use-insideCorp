...
exports.getTargetHourPrefixes = getTargetHourPrefixes;
 ...
 */
function getTargetHourPrefixes(now) {
    const prefixes = [];
...
async function ingestLogHandler(myTimer, context) {
    ...
    for (const blobPath of targetBlobs) {
        try {
            await processOneDiagnosticBlob(blobPath, diagnosticContainer, cursorContainer, poisonContainer, queueClient, config, context);
        }
        catch (err) {
 ...
 */
async function processOneDiagnosticBlob(blobPath, diagnosticContainer, cursorContainer, poisonContainer, queueClient, config, context) {
    // ステップ2: カーソル読取り
    const cursorState = await (0, cursorManager_js_1.readCursor)(cursorContainer, blobPath);
    ...
    const { lines, processedBytes } = (0, diagnosticParser_js_1.parseDiagnosticLines)(data, context);
    if (processedBytes === 0) {
    ...
    for (const line of lines) {
        ...
        if (messageBytes > config.queueMessageMaxBytes) {
            context.warn(`ingestLog: Message size ${messageBytes} exceeds threshold ${config.queueMessageMaxBytes} for shareId=${record.shareId}. Writing directly to Poison Blob.`);
            try {
                await (0, poisonBlobWriter_js_1.writePoisonBlob)(poisonContainer, 'ingestLog', `Message size ${messageBytes} exceeds queue limit ${config.queueMessageMaxBytes}`, record, {
        ...
        try {
            ...
            context.error(`ingestLog: Failed to send message to queue for shareId=${record.shareId}: ${queueErr instanceof Error ? queueErr.message : String(queueErr)}`);
    ...
    try {
        await (0, cursorManager_js_1.writeCursor)(cursorContainer, blobPath, newOffset);
        context.log(`ingestLog: Updated cursor for ${blobPath}: offset=${newOffset}`);
    }
    catch (cursorErr) {
        context.error(`ingestLog: Failed to update cursor for ${blobPath}: ${cursorErr instanceof Error ? cursorErr.message : String(cursorErr)}`);
 ...
 */
functions_1.app.timer('ingestLog', {
...
//# sourceMappingURL=ingestLog.js.map