...
exports.poisonHandlerFunction = poisonHandlerFunction;
 ...
 */
async function poisonHandlerFunction(queueItem, context) {
    ...
    try {
        ...
        if (typeof queueItem === 'string') {
            try {
                ...
                if (typeof rawMessage === 'object' && rawMessage !== null && 'shareId' in rawMessage) {
                    shareId = String(rawMessage.shareId);
                }
            }
            catch {
        ...
        const dequeueCount = context.triggerMetadata?.dequeueCount ?? 0;
        ...
        const poisonBlobPath = await (0, poisonBlobWriter_js_1.writePoisonBlob)(poisonContainer, 'processQueue', 'Message exceeded maxDequeueCount and was moved to poison queue.', rawMessage, {
        ...
        context.error(`poisonHandler: CRITICAL - Failed to process poison message: ${err instanceof Error ? err.message : String(err)}`);
 ...
 * ユーザー側でのプロビジョニングは不要。
 */
functions_1.app.storageQueue('poisonHandler', {
    queueName: 'buffer-poison',
    connection: '',
    handler: poisonHandlerFunction,
});
//# sourceMappingURL=poisonHandler.js.map