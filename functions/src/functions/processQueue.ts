...
import { toLogAnalyticsRecord } from '../lib/logRecord.js';
import type { LogRecord } from '../lib/logRecord.js';

/**
 * processQueue のメインハンドラー
 */
export async function processQueueHandler(
  queueItem: unknown,
  ...
  try {
    if (typeof queueItem === 'string') {
      record = JSON.parse(queueItem) as LogRecord;
    } else {
      record = queueItem as LogRecord;
    }
  } catch (err) {
    throw new Error(
      `processQueue: Failed to parse queue message: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  try {
    await blockBlobClient.upload(
      analyticsContent,
      Buffer.byteLength(analyticsContent, 'utf-8'),
      {
    ...
    throw new Error(
      `processQueue: Failed to write Analytics JSON for shareId=${record.shareId}: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  try {
    const laRecord = toLogAnalyticsRecord(record);
    await sendToLogAnalytics([laRecord]);
    ...
    throw new Error(
      `processQueue: Failed to send to Log Analytics for shareId=${record.shareId}: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
