...
import { buildCursorBlobName, readCursor, writeCursor } from '../lib/cursorManager.js';
...
describe('cursorManager', () => {
  describe('buildCursorBlobName', () => {
    it('should append .json to the diagnostic blob path', () => {
      ...
      expect(buildCursorBlobName(path)).toBe(path + '.json');
    });
  });
  ...
  describe('readCursor', () => {
    it('should read existing cursor state', async () => {
      ...
      const result = await readCursor(mockContainerClient, 'test/PT1H.json');
  ...
  describe('writeCursor', () => {
    it('should write cursor state to blob', async () => {
      ...
      await writeCursor(mockContainerClient, 'test/PT1H.json', 5000);
    ...
    it('should throw on write failure (caller handles)', async () => {
      ...
      await expect(
        writeCursor(mockContainerClient, 'test/PT1H.json', 5000),
