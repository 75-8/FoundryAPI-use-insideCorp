...
import { getTargetHourPrefixes } from '../functions/ingestLog.js';
...
describe('ingestLog', () => {
  ...
  describe('getTargetHourPrefixes', () => {
    it('should return 2 prefixes (current hour + previous hour)', () => {
      ...
      const prefixes = getTargetHourPrefixes(now);
  ...
  describe('ingestLogHandler', () => {
    it('should skip when no diagnostic blobs exist', async () => {
      ...
      await ingestLogHandler({} as any, context);

      expect(context.log).toHaveBeenCalledWith(
        expect.stringContaining('No target diagnostic blobs found'),
      );
    });
    ...
    it('should advance cursor only through successfully handled lines when queue send fails (§11)', async () => {
      ...
      const currentPrefix = getTargetHourPrefixes(new Date())[0];
      ...
      getBlobServiceClient.mockReturnValue({
        getContainerClient: jest.fn((name: string) => {
          if (name === 'insights-logs-gatewaylogs') return diagnosticContainer;
          if (name === 'cursor') return cursorContainer;
          if (name === 'poison') return poisonContainer;
          return {};
        }),
      });
      ...
      expect(writeCursor).not.toHaveBeenCalledWith(
        cursorContainer,
        blobPath,
        1000 + line1Bytes + line2Bytes,
      );
    });
  });
